    ...
    while (True):
        try:
            ...
            if record:
                ...
                exp_status = False
                try:
                    result,front,side,back,f_bd_features,s_bd_features,b_bd_features = process_outline(db, record['bbiid'])
                    logger.info('processing record %s, result:%s'% (record, result))
                    if result == 4:
                        db.update_new_outline(record['bbiid'],json.dumps(front),json.dumps(side),json.dumps(back),json.dumps(f_bd_features),json.dumps(s_bd_features),json.dumps(b_bd_features))
                    ...
                    if retries>=3:
                        db.update_new_outline_result(record['bbiid'], 5, 'retries:%d, exception trace:%s'%(retries,msg))
                    else:
                        db.update_new_outline_result(record['bbiid'], retries+1, 'retries:%d, exception trace:%s' % (retries, msg))
