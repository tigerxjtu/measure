...
df['front']=df['front_hip']*df['身高']/df['fh']
df['front'].describe()

df['side']=df['side_hip']*df['身高']/df['sh']
df['side'].describe()

df['back']=df['back_hip']*df['身高']/df['bh']
df['back'].describe()

X=df[['front','side','back']]
df['front_min']=df[['front','back']].apply(min,axis=1)
X=df[['front_min','side']]
df['tun']=df['臀围']-tun_mean
Y=df['tun']
...
df['diff']=diff
df.to_excel(os.path.join(dir,'hip_output_3side_linear2.xlsx'))

from sklearn.externals import joblib

joblib.dump(value=model, filename=os.path.join(dir,'hip_3side_linear2.model'))
