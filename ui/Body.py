...
from BodyFeature import min_angle_feature, min_feature, min_distance_angle, round_int, min_y_pt, get_outline_distribute, \
    speed_delta, range_outline, feature_by_min, foot_front_feature
...
class Body(object):
    ...
    def get_main_distances(self):
        ...
        up_distances = [x_inersect_y(right, bot_right, y) - x_inersect_y(left, bot_left, y) for y in
                        range(y_start, y_up)]
    ...
    def load_outline(self):
        ...
        try:
            ...
            save_outline(self.img_file, self.outline_file)
            self.img_w, self.img_h, self.outline = get_points(self.outline_file)
    ...
    def load_export_features(self):
        ...
        if self.tag == 'F':
            ...
            self.features['f_shoulder_L'], self.features['f_shoulder_R'] = auto_features['shoulder_L'], auto_features[
                'shoulder_R']
            ...
            if 'left_foot' in auto_features:
                self.features['left_foot']=auto_features['left_foot']
            if 'right_foot' in auto_features:
                self.features['right_foot'] = auto_features['right_foot']
        ...
        if self.tag == 'S':
            ...
            self.features['s_neck_up_L'], self.features['s_neck_up_R'] = auto_features['neck_L'], auto_features[
                'neck_R']
    ...
    def calculate_features(self):
        ...
        if self.tag == 'F':
            left_foot, right_foot = foot_front_feature(self.outline, features['huiyin'][1], self._max_y,
                                                       features['huiyin'][0])
            # print(left_foot,right_foot)
            if left_foot:
                features['left_foot'] = left_foot
                # print(self._max_y - left_foot[1])
            if right_foot:
                features['right_foot'] = right_foot
                # print(self._max_y - right_foot[1])
    ...
    def proces_neck_feature(self):
        ...
        if self.tag == 'S':
            ...
            result = feature_by_min(self.outline, low, up, self._min_y, self._max_y)
            if result:
                # print(result)
